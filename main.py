...
def open_popup():
    tkinter.messagebox.showinfo('How it Works?', 'When you input a movie then CineHub given a recomendation using IMDb dataset!')

def developers():
    tkinter.messagebox.showinfo('Developers',
        '''
        1. Mahfuz Mia \n
        2. Azmain Hossen Daiyan \n
        3. Afiya Amdad Rami \n
        4. ----------------
        '''                            
    )
...
root.title("CineHub v0.1.0")
root.geometry('960x720')  # width x height
root.resizable(width=False, height=False)  # restricts window size

# creating menu widget
menu = Menu(root)
helpMenu = Menu(menu, tearoff=0, postcommand=partial(play_menu_sound, 'menu_bar'), font='Courier 14', bg='yellow',
                activebackground='black', activeforeground='white')
menu.add_cascade(label='Menu', menu=helpMenu)
helpMenu.add_command(label='How it Works?', command=open_popup)
helpMenu.add_separator()
helpMenu.add_command(label='See Developers...', command= developers)
helpMenu.add_separator()
helpMenu.add_command(label='See on github...', command=lambda: webbrowser.open_new("https://github.com/mahfuz1703/CineHub"))
helpMenu.add_separator()
...
# creating widgets
label1 = Label(frame, font='Courier 13 italic', text='Select a Movie for get recomendation!', height=2, width=65,
